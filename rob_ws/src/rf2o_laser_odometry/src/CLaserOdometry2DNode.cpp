...
CLaserOdometry2DNode::CLaserOdometry2DNode(): Node("CLaserOdometry2DNode")
{
  ...
  // ANTIGRAVITY FIX: Use Reliable QoS to match standard ROS 2 publishers (like ldlidar default)
  laser_sub = this->create_subscription<sensor_msgs::msg::LaserScan>(laser_scan_topic, rclcpp::QoS(rclcpp::KeepLast(10)).reliable().durability_volatile(),
      std::bind(&CLaserOdometry2DNode::LaserCallBack, this, std::placeholders::_1));
  
  RCLCPP_INFO(get_logger(), "Params: laser_topic='%s', odom_topic='%s', init_pose_topic='%s'", 
              laser_scan_topic.c_str(), odom_topic.c_str(), init_pose_from_topic.c_str());

  // Initialize pose
  if (init_pose_from_topic != "")
  {
    RCLCPP_INFO(get_logger(), "Waiting for init pose on topic: %s", init_pose_from_topic.c_str());
  ...
  {
    RCLCPP_INFO(get_logger(), "No init pose topic set. Initializing to (0,0,0) immediately.");
    // init to 0
